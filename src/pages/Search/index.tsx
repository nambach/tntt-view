...
import {Alert, Button, Grid, InputAdornment, Stack, TextField} from '@mui/material';
      ...
      <Grid item xs={12} sm={8}>
        <Stack margin={4} spacing={6}>
          <Stack spacing={2}>
            ...
            <Button variant="contained" onClick={() => setSearch(name)}>Tìm</Button>
            <Stack direction='row' spacing={2} justifySelf='stretch'>
              <Alert severity="warning">Website chưa hỗ trợ tìm theo tên Thánh.</Alert>
            </Stack>
          </Stack>
          {
            (data?.length ? <ResultTable students={data}/>:(search && !isFetching ?
